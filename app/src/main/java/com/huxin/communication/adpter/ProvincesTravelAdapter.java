    ...
    public BodyViewHoder onCreateViewHolder(ViewGroup parent, int viewType) {
        ...
        Hoder.mRelativeLayout.setOnClickListener(new View.OnClickListener() {
            ...
            public void onClick(View v) {
                if (type == 4) {
                    PreferenceUtil.putString(Constanst.TICKET_PROVINCE_NAME, mList.get(Hoder.getAdapterPosition()).getProvince_name());
                    PreferenceUtil.putString(Constanst.TICKET_PROVINCE_CODE,mList.get(Hoder.getAdapterPosition()).getProvince_code());
                    mActivity.finish();
                } else {
